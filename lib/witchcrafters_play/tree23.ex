...
#  endinsert(left, orderable, value)
  ...
  def insert(tree = %Node2{lower_key: lower_key, upper_key: upper_key, left: %Leaf{} = left, right: %Leaf{} = right}, orderable, value) do
    case {compare(orderable, lower_key), compare(orderable, upper_key)} do
      ...
      {:lesser, _} -> Node3.new(orderable, lower_key, Leaf.new(orderable, value), left, right)
      {_, :greater} -> Node3.new(lower_key, upper_key, left, right, Leaf.new(orderable,value))
      _ -> Node3.new(lower_key, orderable, left, Leaf.new(orderable, value), right)
    end
  end

  def insert(tree = %Node3{lower_key: lower_key, upper_key: upper_key, left: %Leaf{} = left, middle: %Leaf{} = middle,
    right: %Leaf{} = right}, orderable, value) do
    case {compare(orderable, lower_key), compare(orderable, upper_key)} do
      {:equal, _} -> struct_put_in(tree, [:left, :value], value)
      {_, :equal} -> struct_put_in(tree, [:middle, :value], value)
      {:lesser, _} -> Node4.new(orderable, lower_key, upper_key, Leaf.new(orderable, value), left, middle, right)
      {:greater, :lesser} -> Node4.new(lower_key, orderable, upper_key, left, Leaf.new(orderable, value), middle, right)
      {_, :greater} -> case compare(orderable, right.key) do
                         :equal -> struct_put_in(tree, [:right, :value], value)
                         :greater -> Node4.new(lower_key, upper_key, right.key, left, middle, right, Leaf.new(orderable, value))
                         :lesser -> Node4.new(lower_key, upper_key, orderable, left, middle, Leaf.new(orderable, value), right)
                       end
    end
  end

  def insert(tree = %Node2{lower_key: lower_key, upper_key: upper_key, left: left, right: right}, orderable, value) do
    case compare(orderable, lower_key) do
      :lesser -> case new_node = insert(left, orderable, value) do
        %Node4{lower_key: lk, middle_key: mk, upper_key: uk, left: l,
          lower_middle: lm, upper_middle: um, right: r} ->
                     Node3.new(lk, mk, Node2.new(lk, mk, left, lm), Node2.new(mk, uk, um, r),
                       right)
      ...
      :greater -> case new_node = insert(right, orderable, value) do
          %Node4{lower_key: lk, middle_key: mk, upper_key: uk, left: l,
            lower_middle: lm, upper_middle: um, right: r} ->
            Node3.new(lower_key, upper_key, left, Node2.new(lk, mk, l, lm), Node2.new(mk,  uk, um, r))
          _ -> Map.put(tree, :right,  new_node)
          end
    end
  end
